...
jQuery(document).ready(function(){
    if( typeof direction_l10n.map == "object" && direction_l10n.map != null ){
        loadDirectionsMap(
            direction_l10n.map.id,
            direction_l10n.map.styling_hue,
            direction_l10n.map.styling_saturation,
            direction_l10n.map.styling_lightness,
            direction_l10n.map.styling_gamma,
            direction_l10n.map.zoom,
            direction_l10n.map.type,
            direction_l10n.map.bike_layer,
            direction_l10n.map.traffic_layer,
            direction_l10n.map.transit_layer
        );
    }

});

function loadDirectionsMap(id, hue, saturation, lightness, gamma, zoom, type, bike, traffic, transit) {
    data = {
    ...
    jQuery.ajax({
        url: direction_l10n.ajax_url,
        dataType: 'json',
        method: 'post',
        data: data,
        beforeSend: function () {}
    }).done(function (response) {
        HGinitializeDirectionMap(response);
    }).fail(function () {
        console.log('Failed to load response from database');
    });

    function HGinitializeDirectionMap(response){
        if( !response.success ){
            return false;
        }
        ...
        for (var i = 0; i < maps.length; i++) {
            ...
            google.maps.event.addListener(mapdirection, 'rightclick', function (event) {
                hgGmapsPlaceDirection(event.latLng);
            });
            ...
            google.maps.event.addListener(autocomplete_start, 'place_changed', function () {
                ...
                geocoder.geocode({'address': addr}, function (results, status) {
                    ...
                    if( newDirection ){
                        ...
                        hgGmapsPlaceDirection( results[0].geometry.location );
                        hgGmapsupdateDirectionInputsStart( results[0].geometry.location );
                    }
            ...
            jQuery(".direction_options_input").on("change",function(){
                ...
                if( tryShowSteps != newDirectionShowSteps ){
                    ...
                    hgGmapsShowHideSteps( newDirection.directions, tryShowSteps, mapdirection );
                }
                ...
                if( tryMode != newDirectionMode ){
                    directionsService.route({
                        ...
                        if (status == google.maps.DirectionsStatus.OK) {
                            ...
                            hgGmapsShowNotice( direction_l10n.invalid_direction_points );
                        }
                    });
                }
            ...
            jQuery(".edit_direction_list_delete a").on("click",function(){
                ...
                for (var e = 0; e < info_directions.length; e++) {
                        ...
                        directionsService.route(request, function(response, status) {
                            if (status == google.maps.DirectionsStatus.OK) {
                                ...
                                editDirection.addListener('directions_changed', function() {
                                    hgGmapsupdateEditDirectionInputsDirDrag(editDirection.getDirections());
                                    hgGmapsEditShowHideSteps( editDirection.directions, editDirectionShowSteps, map_direction_edit );
                                });
                    ...
                    google.maps.event.addListener(map_direction_edit, 'rightclick', function (event) {
                        ...
                        hgGmapsPlaceEditDirection(event.latLng);
                    });
                    ...
                    jQuery(".direction_edit_options_input").on("change",function(){
                        ...
                        if( tryShowSteps != editDirectionShowSteps ){
                            ...
                            hgGmapsShowHideSteps( editDirection.directions, tryShowSteps, map_direction_edit );
                        }
...
function hgGmapsPlaceEditDirection(latlng){
    editDirectionMode = jQuery("#direction_edit_travelmode").val();
    var request = {
    ...
    (function(latlng){
        directionsService.route(request, function(response, status) {
            ...
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                hgGmapsupdateEditDirectionInputsEnd( editDirectionCoords[1] );
...
function hgGmapsupdateEditDirectionInputsEnd(latlng){
...
function hgGmapsupdateEditDirectionInputsDirDrag(event){
    editDirectionCoords = [
        {
...
function hgGmapsEditShowHideSteps( direction, show, map ){
    if(show){
        if(editDirectionMarkers){
...
function hgGmapsShowHideSteps( direction, show, map ){
    if(show){
...
function HgGmapsAttachInstructionText(stepDisplay, marker, text, map) {
...
function hgGmapsPlaceDirection(latlng){
    if ( newDirectionCoords.length == 2 ){
        hgGmapsPlaceEndPoint(latlng);
        return false;
    }

    if( !newDirectionStartMarker ){
        hgGmapsupdateDirectionInputsStart(latlng);
        newDirectionStartMarker = new google.maps.Marker({
            draggable: true,
            position: latlng,
            title: direction_l10n.start_point_title,
            map: mapdirection
        });
        google.maps.event.addListener(newDirectionStartMarker, "drag", function (event) {
            hgGmapsupdateDirectionInputsStart( event.latLng );
        });
        hgGmapsupdateDirectionInputsStart( latlng );
    }else{
        hgGmapsPlaceEndPoint(latlng);
...
function hgGmapsPlaceEndPoint( latlng ){
    newDirectionCoords = [
        {
    ...
    hgGmapsCreateDirection();
}

function hgGmapsCreateDirection(){
    ...
    directionsService.route(request, function(response, status) {
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            newDirection.addListener('directions_changed', function() {
                hgGmapsupdateDirectionInputsDirDrag(newDirection.getDirections());
                hgGmapsShowHideSteps( newDirection.directions, newDirectionShowSteps, mapdirection );
            });

            hgGmapsupdateDirectionInputsEnd( newDirectionCoords[1] );
...
function hgGmapsupdateDirectionInputsStart( latlng ){
...
function hgGmapsupdateDirectionInputsEnd( latlng ){
...
function hgGmapsupdateDirectionInputsDirDrag(event){
    newDirectionCoords = [
        {
            lat : event.routes[0].legs[0].start_location.lat(),
            lng : event.routes[0].legs[0].start_location.lng()
