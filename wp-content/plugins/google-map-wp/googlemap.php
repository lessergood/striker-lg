...
function hg_gmaps_admin_scripts($hook) {
	...
	if( $hook == $hg_gmaps_admin_pages['main_page'] ){
		...
		wp_enqueue_script( 'maps_directions_js', plugins_url( 'js/admin.directions.js', __FILE__ ), array( 'jquery' ), true );
		...
		wp_enqueue_script( 'maps_directions_js', plugins_url( 'js/admin.directions.js', __FILE__ ), array( 'jquery' ), true );
		wp_localize_script( 'maps_directions_js', 'direction_l10n', array(
			'map'=>$map,
			'ajax_url'=>admin_url( 'admin-ajax.php' ),
			'hg_gmaps_nonce' => wp_create_nonce('hg_gmaps_nonce'),
			'start_point_title' => __( 'Start Point', 'hg_gmaps' ),
			'invalid_direction_points' => __( 'Could not construct a route with the given options', 'hg_gmaps' )
		) );

		wp_enqueue_script( 'maps_circle_js', plugins_url( 'js/admin.circle.js', __FILE__ ), array( 'jquery' ), true );
		wp_localize_script( 'maps_circle_js', 'circle_l10n', array(
			'map'=>$map,
			'ajax_url'=>admin_url( 'admin-ajax.php' ),
			'hg_gmaps_nonce' => wp_create_nonce('hg_gmaps_nonce')
		) );
		...
		switch($_REQUEST['task']){
			...
			case "submit_general_options":
				...
				$sql = $wpdb->prepare( "UPDATE " . $wpdb->prefix . "g_maps SET name=%s,type=%s,zoom=%s,border_radius=%s,center_lat=%s,center_lng=%s,pan_controller=%s,zoom_controller=%s,type_controller=%s,scale_controller=%s,street_view_controller=%s,overview_map_controller=%s,width=%s,height=%s,align=%s,info_type=%s,wheel_scroll=%s,draggable=%s,language=%s,min_zoom=%s,max_zoom=%s,animation=%s WHERE id=%s",
					$_POST['map_name'], $_POST['map_type'], $_POST['map_zoom'], $_POST['map_border_radius'], $_POST['map_center_lat'], $_POST['map_center_lng'], $_POST['map_controller_pan'], $_POST['map_controller_zoom'], $_POST['map_controller_type'], $_POST['map_controller_scale'], $_POST['map_controller_street_view'], $_POST['map_controller_overview'], $_POST['map_width'], $_POST['map_height'], $_POST['map_align'], $_POST['map_infowindow_type'], $_POST['wheel_scroll'], $_POST['draggable'], $_POST['map_language'], $_POST['min_zoom'], $_POST['max_zoom'], $_POST['map_animation'], $_POST['id'] );
				if ( $wpdb->query( $sql ) ) {
					$map_params = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "g_maps WHERE id=%s", $_POST['id'] ) );
					foreach ( $map_params as $param ) {
						echo json_encode( array(
					...
					if ( $getDirections ) {
						foreach ( $getDirections as $direction ) {
							$response['directions'][] = array(
								...
								'show_marker'        => $direction->show_marker,
							);
						}
					}
