...
class UPME_Profile_Header {
    ...
    public function get_cover_image(){
        ...
        $cover_image_default = apply_filters('upme_default_cover_image', $cover_image_default, $id, $view);
...
//        }
    ...
    public function default_profile_header($params){
        ...
        $upme_featured_member_level = get_user_meta($id,'upme_featured_member_level',true);
        $upme_featured_member_status = isset($upme->upme_options['featured_members_enabled_status']) ? $upme->upme_options['featured_members_enabled_status'] : '0';
        $upme_featured_member_color = isset($upme->upme_options['featured_member_level_'.$upme_featured_member_level.'_color']) ? $upme->upme_options['featured_member_level_'.$upme_featured_member_level.'_color'] : '';
        $upme_featured_member_color = 'background:'.$upme_featured_member_color;

        $form_name_class = '';
        if(isset($upme->profile_form_name) && $upme->profile_form_name  != ''){
            $form_name_class = 'upme-'.$upme->profile_form_name;
        }

        if($upme_featured_member_status == '1'){
            $display .= '<div style="'.$upme_featured_member_color.'" upme-data-user-id="'.$id.'" class="upme-head '.$form_name_class.'  upme_featured_level-'.$upme_featured_member_level.' ">
                        <div class="upme-left">';
        }else{
            $display .= '<div class="upme-head '.$form_name_class.'" upme-data-user-id="'.$id.'" >
                        <div class="upme-left">';
        }
        ...
        if ($upme->can_edit_profile($upme->logged_in_user, $id)) {
            ...
            $display .= $this->profile_title_display;
            $display.= apply_filters('upme_after_profile_title','', array('user_id' => $id ));
